...
import { NusicJettonMinter, NusicJettonMinterContent, jettonContentToCell, jettonMinterConfigToCell } from '../wrappers/NusicJettonMinter';
...
export async function run(provider: NetworkProvider) {
    ...
    let contentUrl = "https://coral-wasteful-lion-960.mypinata.cloud/ipfs/QmZ7RqF2XNGoA4vF2soLEN6y8VBm4x3STByJ69WYJMP1VQ/metadata.json";
    ...
    const wallet_code = await compile('NusicJettonWallet');

    const minter  = NusicJettonMinter.createFromConfig({admin,
                                                  content,
                                                  wallet_code,
                                                  }, 
                                                  await compile('NusicJettonMinter'));
