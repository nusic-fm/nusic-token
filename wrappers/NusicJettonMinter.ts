...
import { Op } from './NusicJettonConstants';

export type NusicJettonMinterContent = {
    type:0|1,
    uri:string
};

export type NusicJettonMinterConfig = {admin: Address; content: Cell; wallet_code: Cell};

export function jettonMinterConfigToCell(config: NusicJettonMinterConfig): Cell {
    return beginCell()
                      .storeCoins(0)
...
export function jettonContentToCell(content:NusicJettonMinterContent) {
    return beginCell()
...
export class NusicJettonMinter implements Contract {
    constructor(readonly address: Address, readonly init?: { code: Cell; data: Cell }) {}

    static createFromAddress(address: Address) {
        return new NusicJettonMinter(address);
    }

    static createFromConfig(config: NusicJettonMinterConfig, code: Cell, workchain = 0) {
        const data = jettonMinterConfigToCell(config);
        const init = { code, data };
        return new NusicJettonMinter(contractAddress(workchain, init), init);
    }
    ...
    async sendMint(provider: ContractProvider, via: Sender, to: Address, jetton_amount: bigint, forward_ton_amount: bigint, total_ton_amount: bigint) {
        ...
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: NusicJettonMinter.mintMessage(this.address, to, jetton_amount, forward_ton_amount, total_ton_amount),
    ...
    */
    ...
    async sendDiscovery(provider: ContractProvider, via: Sender, owner: Address, include_address: boolean, value:bigint = toNano('0.1')) {
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: NusicJettonMinter.discoveryMessage(owner, include_address),
            value: value,
        });
    }
    ...
    async sendChangeAdmin(provider: ContractProvider, via: Sender, newOwner: Address) {
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: NusicJettonMinter.changeAdminMessage(newOwner),
            value: toNano("0.05"),
        });
    }
    ...
    async sendChangeContent(provider: ContractProvider, via: Sender, content: Cell) {
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: NusicJettonMinter.changeContentMessage(content),
            value: toNano("0.05"),
        });
    }
