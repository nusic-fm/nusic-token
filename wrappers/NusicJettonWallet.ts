...
export type NusicJettonWalletConfig = {};

export function jettonWalletConfigToCell(config: NusicJettonWalletConfig): Cell {
    return beginCell().endCell();
}

export class NusicJettonWallet implements Contract {
    constructor(readonly address: Address, readonly init?: { code: Cell; data: Cell }) {}

    static createFromAddress(address: Address) {
        return new NusicJettonWallet(address);
    }

    static createFromConfig(config: NusicJettonWalletConfig, code: Cell, workchain = 0) {
        const data = jettonWalletConfigToCell(config);
        const init = { code, data };
        return new NusicJettonWallet(contractAddress(workchain, init), init);
    }
    ...
    async sendTransfer(provider: ContractProvider, via: Sender,
        ...
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: NusicJettonWallet.transferMessage(jetton_amount, to, responseAddress, customPayload, forward_ton_amount, forwardPayload),
            value:value
        });
    ...
    */
    ...
    async sendBurn(provider: ContractProvider, via: Sender, value: bigint,
        ...
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: NusicJettonWallet.burnMessage(jetton_amount, responseAddress, customPayload),
            value:value
        });
    ...
    */
    ...
    async sendWithdrawTons(provider: ContractProvider, via: Sender) {
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: NusicJettonWallet.withdrawTonsMessage(),
            value:toNano('0.1')
        });
    ...
    */
    ...
    async sendWithdrawJettons(provider: ContractProvider, via: Sender, from:Address, amount:bigint) {
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: NusicJettonWallet.withdrawJettonsMessage(from, amount),
            value:toNano('0.1')
        });
